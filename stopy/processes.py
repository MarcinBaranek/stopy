...
class OrnsteinUhlenbeck:
    ...
    EXAMPLES:
        # generate process to plot
        to_plot = OrnsteinUhlenbeck.generate_array(
            np.zeros(shape=(3,)), x_axis=True)
        plt.plot(to_plot[0], to_plot[1])
        plt.show()

        # generate process on grid
        grid = np.array([0.0, 1.9, 10.0, 30.0, 30.1, 32])
        plt.plot(grid, OrnsteinUhlenbeck.generate_on_grid(
            grid, start_point=np.zeros(shape=(4,))))
        plt.show()
    ...
    def _eval_first_part(theta=1, mu=0, start_point=None, time=1):
        ...
        :param mu: numpy.array with the same shape as start_point,
            it is a mean value of process
        :param start_point: numpy.array with the same shape as mu,
            it is a initial value
    ...
    def _eval(theta, mu, sigma, start_point, time, wiener):
        return OrnsteinUhlenbeck._eval_first_part(
            theta=theta, mu=mu, start_point=start_point, time=time)\
