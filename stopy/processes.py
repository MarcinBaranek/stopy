...
class Poisson:
    ...
    def generate_on_grid(_grid, lam=1, start_point=None):
        ...
        :return: array with realizations of the Poisson process on grid
         """
        _array = [start_point] if start_point is None else np.zeros(shape=(1,))
        for i in range(1, len(_grid), 1):
...
class OrnsteinUhlenbeck:
    ...
    def _eval(theta, mu, sigma, start_point, time, wiener):
        # print(np.sqrt(2 * theta))
        # print(sigma * np.exp(-theta * time) * wiener / np.sqrt(2 * theta))
    ...
    def generate(theta=1, mu=0, sigma=1, start_point=None, time=1, division=500):
        ...
        """
        yield start_point if start_point is not None else np.zeros(shape=(1,))
        _time, step, _wiener = 0.0, 1 / division, np.zeros(shape=start_point.shape)
        if time < 0:
            print(60*"=")
            print(theta)
            while True:
                _wiener, start_point, _time = OrnsteinUhlenbeck._eval_step(theta, mu, sigma, start_point,
                                                                           _time, _wiener, step)
                yield start_point
        else:
            for _ in range(int(time * division)):
    ...
    def generate_on_grid(_grid, start_point=np.zeros(shape=(1,))):
        """ function generating the process on grid     ToDo update this!!!
        :param _grid: sorted list or array with grid
        :param start_point: numpy.array with shape (n,), starting point of the process
        :return: array with realizations of the Wiener process on grid
         """
